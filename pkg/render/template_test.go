...
func TestRender(t *testing.T) {
	tests := []struct {
		...
		name: "randNorm",
		text: `{{randNorm 50 2}}-{{randNormInt 50 2}}-{{randLogNorm 50 2}}-{{randLogNormInt 50 2}}`,
		verify: func(t *testing.T, s string) {
			assert.NotEmpty(t, s)
		},
	}, {
		...
		name: "arange",
		text: `{{ range $i := (arange 1 10 1)}}{{$i}}{{ end }}`,
		verify: func(t *testing.T, s string) {
			assert.Equal(t, "12345678910", s)
		},
	}, {
		name: "arangeIP, simple",
		text: `{{ range $i := (arangeIP "192.168.1.1" 3)}}{{$i}}{{ end }}`,
		verify: func(t *testing.T, s string) {
			assert.Equal(t, "192.168.1.1192.168.1.2192.168.1.3", s)
		},
	}, {
		name: "arangeIP, across 3 and 4",
		text: `{{ range $i := (arangeIP "192.168.1.254" 4)}}{{$i}}{{ end }}`,
		verify: func(t *testing.T, s string) {
			assert.Equal(t, "192.168.1.254192.168.1.255192.168.2.1192.168.2.2", s)
		},
	}, {
		name: "arangeIP, across 2 and 3",
		text: `{{ range $i := (arangeIP "192.168.255.254" 4)}}{{$i}}{{ end }}`,
		verify: func(t *testing.T, s string) {
			assert.Equal(t, "192.168.255.254192.168.255.255192.169.1.1192.169.1.2", s)
		},
	}, {
		name: "arangeIP, invalid start IP",
		text: `{{ range $i := (arangeIP "192.168.255" 4)}}{{$i}}{{ end }}`,
		verify: func(t *testing.T, s string) {
			assert.Equal(t, "192.168.255", s)
		},
	}, {
		name: "arangeIP, invalid start IP, not number",
		text: `{{ range $i := (arangeIP "a.b.c.d" 4)}}{{$i}}{{ end }}`,
		verify: func(t *testing.T, s string) {
			assert.Equal(t, "a.b.c.d", s)
		},
	}, {
		name: "arangeIP, a lot",
		text: `{{ len (arangeIP "192.168.1.1" 865) }}`,
		verify: func(t *testing.T, s string) {
			assert.Equal(t, "865", s)
		},
	}}
