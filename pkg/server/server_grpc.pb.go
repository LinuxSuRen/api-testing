...
type AIExtensionClient interface {
...
func NewAIExtensionClient(cc grpc.ClientConnInterface) AIExtensionClient {
	return &aIExtensionClient{cc}
}

func (c *aIExtensionClient) GenerateContent(ctx context.Context, in *GenerateContentRequest, opts ...grpc.CallOption) (*GenerateContentResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(GenerateContentResponse)
	err := c.cc.Invoke(ctx, AIExtension_GenerateContent_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
//
// AIExtension service provides AI-powered capabilities for atest.
type AIExtensionServer interface {
	// Generates content based on natural language prompts with context.
	// This is a general-purpose AI interface that can handle SQL generation,
	// test case writing, mock service creation, and other AI-powered tasks.
...
// UnimplementedAIExtensionServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
type UnimplementedAIExtensionServer struct{}
...
// UnsafeAIExtensionServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to AIExtensionServer will
// result in compilation errors.
type UnsafeAIExtensionServer interface {
	mustEmbedUnimplementedAIExtensionServer()
}

func RegisterAIExtensionServer(s grpc.ServiceRegistrar, srv AIExtensionServer) {
	// If the following call pancis, it indicates UnimplementedAIExtensionServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
	...
	s.RegisterService(&AIExtension_ServiceDesc, srv)
}

func _AIExtension_GenerateContent_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GenerateContentRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AIExtensionServer).GenerateContent(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: AIExtension_GenerateContent_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AIExtensionServer).GenerateContent(ctx, req.(*GenerateContentRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var AIExtension_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "server.AIExtension",
	HandlerType: (*AIExtensionServer)(nil),
	Methods: []grpc.MethodDesc{
		{
